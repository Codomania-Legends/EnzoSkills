    ...
    return (
        <div key={key} className={`bg-[#ffffff25] h-full p-2 rounded-xl flex w-full  justify-evenly items-start absolute top-0 left-0 transition-all duration-300 ${key === index ? "come-in-front z-10 opacity-100" : "go-back z-0 pointer-events-none"}`} onClick={onClickFunction}>
            ...
            <div className='flex gap-2 h-full flex-col justify-evenly items-end w-[20%]'>
                <a href={repo} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className='text-[0.5rem] font-semibold font-[Manrope] bg-[#534DB4] text-white px-2 py-1 rounded-full w-22 flex items-center justify-center h-5'>Repository</a>
                <a href={demo} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className='text-[0.5rem] font-semibold font-[Manrope] bg-white text-[#534DB4] px-2 py-1 rounded-full flex items-center justify-center w-22 h-5'>
